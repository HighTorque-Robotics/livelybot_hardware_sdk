...
1. 首先需要安装ROS1。（Ubuntu20需要选择noetic）
...
2. 安装串口通信的相关包。
    ```shell
    sudo apt install libserialport*
    ```
...
1. 克隆代码到本地
    ...
    ```shell
    git clone https://github.com/HighTorque-Robotics/livelybot_hardware_sdk.git
    ```
 ...
 * 赋予当前用户串口权限
   ...
   ```shell
   cat /dev/ttyACM0 # ACM串口
   # 或
   cat /dev/ttyUSB0 # USB串口
   ```
...
2. 连接IMU，检查连接状态：`ls /dev/tty*`，一般为`ttyACM*`或者`ttyUSB*`;
...
### 七、电机控制
1. 连接主控板，检查连接状态：`ls /dev/tty*`，一般为`ttyACM*`或者`ttyUSB*`；
...
3. 创建机器人对象：`livelybot_serial::robot rb`;

4. `rb`的成员变量`Motors`为存放电机对象的容器，包含配置文件描述的所有电机的对象，电机在容器中的位置顺序为`canport0`的一号、二号、三号电机，依次类推，然后是`canport1`的一号、二号、三号电机，依次类推，依次是`canport3`、`canport4`（如果有点话），依次类推；
